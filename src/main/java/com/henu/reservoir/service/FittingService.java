...
public class FittingService {
    ...
    private void fitLevel(
        ...
        for (int i = 0; i < waterAreaDaoList.size(); i++) {
            ...
            if ("遥测水位模型".equals(levelModelName)) {
                ...
                MeasuredResultDao measuredResultDao = measuredResultService.findMeasuredResultByReservoirIdAndDate(rid, date);
                if (measuredResultDao == null) {
                    ...
                    //不需要拟合，直接取数据
                    x[i] = Double.parseDouble(measuredResultDao.getWaterLevel());
                }
            }
